...
public class Turnstile {
	...
	public int[] turnstile() {
		...
		PriorityQueue<Integer> enter = new PriorityQueue<>();
		PriorityQueue<Integer> exit = new PriorityQueue<>();
		int index = 0;
		for(int d:direction) {
			if(d==0) enter.add(index++);
			else  exit.add(index++);
		}
		int prevSec = -1;
		int prevDir = -1;
		...
		while(!enter.isEmpty() || !exit.isEmpty()) {
			if(!enter.isEmpty() && arrTime[enter.peek()] <= prevSec) arrTime[enter.peek()]=prevSec+1;
			if(!exit.isEmpty() && arrTime[exit.peek()] <= prevSec) arrTime[exit.peek()]=prevSec+1;
			if(!enter.isEmpty() && !exit.isEmpty()) {
				if(arrTime[enter.peek()] == arrTime[exit.peek()]) {
					if(arrTime[enter.peek()] - prevSec ==1) {
						if(prevDir == 0) {
							 x = enter.poll();
						}
						else {
