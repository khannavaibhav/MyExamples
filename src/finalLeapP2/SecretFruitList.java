...
public class SecretFruitList {
	...
	public String score() {
		...
		String[] customerList = new String[] {"grapes","orange", "mango","melon","mango", "apple","mango","kiwi"};
		boolean isWinner = false;
		int start = 0;
		for(String[] sl: secretList) {
			int k = 0;
			while(start<customerList.length && k<sl.length) {
				String s = sl[k], c = customerList[start];
				if(sl[k].equals(customerList[start]) || sl[k].equals("anything")) {
					k++;
				}
			...
			if(k<sl.length && start== customerList.length) return "LOSER";
		}
