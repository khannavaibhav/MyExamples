...
public class ShopkeeperSale {
	...
	public static void main(String[] args) {
		ShopkeeperSale s = new ShopkeeperSale();
		System.out.println(s.solution().toString());
	}

	public String solution() {
		
		int[] cell = new int[] {1, 3, 3, 2, 5};
		if(cell.length == 0) return "0";
		if(cell.length == 1) return ""+cell[0];
		
		int left = 0;int pointer = 1;
		int sum = 0;
		List<Integer> unchanged = new ArrayList<>();
		while(left<cell.length) {
			while(pointer<cell.length && cell[pointer]>cell[left]) {
				pointer++;
			}
		...
		return "END "+sum + " "+unchanged;
    }
	
}
/*
A shopkeeper has a sale to complete and has arranged the items being sold in a list.
Starting from the left, the shop keeper rings up each item at its full price less the price of the first 
lower or equally priced item to its right.
